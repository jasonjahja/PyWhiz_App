...
const createUserModuleProgress = async (userId: string) => {
    try {
        ...
        for (const course of courses) {
            const userProgressRef = doc(db, 'user_module_progress', `${userId}_${course.id}`);
            ...
            if (!userProgressSnap.exists()) {
                ...
                console.log(`Initialized watchedVideos for user "${userId}" in module "${course.id}"`);
            } else {
                console.log(`Progress for user "${userId}" in module "${course.id}" already exists.`);
            }
        }
    } catch (error) {
...
const initializeUserProgress = async () => {
    try {
        ...
        const userId = '8hzRnSqWVZXH1zEIvm2Ayrps9442'; // Ganti dengan ID pengguna Anda
